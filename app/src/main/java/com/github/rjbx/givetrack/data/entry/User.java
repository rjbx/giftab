...
@IgnoreExtraProperties public class User implements Entry, Parcelable, Cloneable {
    ...
    public void fromParameterMap(Map<String, Object> map) {
        this.uid = (String) map.get(COLUMN_UID);
        this.userEmail = (String) map.get(COLUMN_USER_EMAIL);
        this.userActive = (int) map.get(COLUMN_USER_ACTIVE) == 1;
        this.userBirthdate = (String) map.get(COLUMN_USER_BIRTHDATE);
        this.userGender = (String) map.get(COLUMN_USER_GENDER);
        this.giveImpact = (String) map.get(COLUMN_GIVE_IMPACT);
        this.giveMagnitude = (String) map.get(COLUMN_GIVE_MAGNITUDE);
        this.giveAnchor = (long) map.get(COLUMN_GIVE_ANCHOR);
        this.giveTiming = (int) map.get(COLUMN_GIVE_TIMING);
        this.glanceAnchor = (long) map.get(COLUMN_GLANCE_ANCHOR);
        this.glanceSince = (int) map.get(COLUMN_GLANCE_SINCE) == 1;
        this.glanceTheme = (int) map.get(COLUMN_GLANCE_THEME);
        this.searchDialog = (int) map.get(COLUMN_SEARCH_DIALOG) == 1;
        this.searchFocus = (int) map.get(COLUMN_SEARCH_FOCUS) == 1;
        this.searchFilter = (int) map.get(COLUMN_SEARCH_FILTER) == 1;
        this.searchCompany = (String) map.get(COLUMN_SEARCH_COMPANY);
        this.searchTerm = (String) map.get(COLUMN_SEARCH_TERM);
        this.searchCity = (String) map.get(COLUMN_SEARCH_CITY);
        this.searchState = (String) map.get(COLUMN_SEARCH_STATE);
        this.searchZip = (String) map.get(COLUMN_SEARCH_ZIP);
        this.searchMinrating = (String) map.get(COLUMN_SEARCH_MINRATING);
        this.searchPages = (String) map.get(COLUMN_SEARCH_PAGES);
        this.searchRows = (String) map.get(COLUMN_SEARCH_ROWS);
        this.giveReset = (boolean) map.get(COLUMN_GIVE_RESET);
        this.searchSort = (String) map.get(COLUMN_SEARCH_SORT);
        this.searchOrder = (String) map.get(COLUMN_SEARCH_ORDER);
        this.recordSort = (String) map.get(COLUMN_RECORD_SORT);
        this.recordOrder = (String) map.get(COLUMN_RECORD_ORDER);
        this.giveStamp = (long) map.get(COLUMN_GIVE_STAMP);
        this.recordStamp = (long) map.get(COLUMN_RECORD_STAMP);
        this.userStamp = (long) map.get(COLUMN_USER_STAMP);
    }
